import { tokens,ether, EVM_REVERT, ETHER_ADDRESS} from './helpers' //have access bc of babel presets
...
contract('Exchange', ([deployer, feeAccount, user1, user2])=> {  //array holds accounts(deployer,reciever, exchange)
    let token
    let exchange
    const feePercent = 10;
	...
	describe('depositing tokens', () => {
		...
		describe('success', () => {
			beforeEach(async () => {  //approve for success only not failure cases
			
		    amount = tokens(10)
			await token.approve(exchange.address, amount, {from: user1})  //need to approve tokens first
    ...
    describe('withdrawing tokens', async() => {
		...
		describe('success', async () => {
			beforeEach(async () => {
                ...
                // Withdraw tokens after have tokens on exchange
        ...
        describe('order actions', async () => {
        	...
        	describe('success', async () => {
        	    ...
        	    it('emits a "Cancel" event', async () => {
        	    	...
        	    	event.amountGet.toString().should.equal(tokens(1).toString(), 'amountGet is correct')
        	    	event.tokenGive.should.equal(ETHER_ADDRESS, 'tokenGive is correct')
        	    	event.amountGive.toString().should.equal(ether(1).toString(), 'amountGive is coorect')
        	    	event.timestamp.toString().length.should.be.at.least(1, 'timestamp is present')
